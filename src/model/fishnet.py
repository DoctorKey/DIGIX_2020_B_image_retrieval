...
class MyFish(nn.Module):
    ...
    def _make_score(self, in_ch, out_ch=1000, has_pool=False):
        ...
        # @wanghy delete the last conv which is used for classification
        if has_pool:
...
class FishNet(nn.Module):
    def __init__(self, block, **kwargs):
        ...
        self.fish = MyFish(block, **kwargs)

        # add a fc for features and another fc for cls
        self.feat_fc = nn.Linear(1056, 512)
        self.fc = nn.Linear(512, kwargs['num_cls'])
        self.feat_map_channel = [1056]
    ...
    def forward(self, x):
        ...
        x = self.conv3(x)
        x = self.pool1(x)
        # raw_score is the result before GAP, and score is the result after gap
        score, raw_score = self.fish(x)
        # 1*1 output
        out = score.view(x.size(0), -1)
        #f = self.feat_bn(self.feat_fc(out))
        f = self.feat_fc(out)
        x = self.fc(f)

        return x, f, [raw_score]
